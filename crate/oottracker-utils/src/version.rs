...
use {
    ...
    std::{iter, process::Stdio, str::FromStr as _},
...
pub async fn bizhawk_latest(client: &reqwest::Client) -> Result<Version, BizHawkError> {
    let remote_version_string = client
        ...
        .json(&BizHawkVersionQuery::build_query(
            biz_hawk_version_query::Variables {},
        ))
        .send()
        .await?
        .error_for_status()?
        .json::<graphql_client::Response<biz_hawk_version_query::ResponseData>>()
        .await?
        .data
        .ok_or(BizHawkError::EmptyResponse)?
        .repository
        .ok_or(BizHawkError::MissingRepo)?
        .latest_release
        .ok_or(BizHawkError::NoReleases)?
        .name
        .ok_or(BizHawkError::UnnamedRelease)?;
    let (major, minor, patch) = remote_version_string
        .split('.')
        .map(u64::from_str)
        .chain(iter::repeat(Ok(0)))
        .next_tuple()
        .expect("iter::repeat produces an infinite iterator");
...
pub async fn check_cli_version(package: &str, version: &Version) {
    ...
    .expect("CLI version output is invalid UTF-8");
    let (cli_name, cli_version) = cli_output
        .trim_end()
        .split(' ')
        .collect_tuple()
        .expect("no space in CLI version output");
    assert_eq!(cli_name, package);
    assert_eq!(
        *version,
        cli_version.parse().expect("failed to parse CLI version")
    );
}
...
pub async fn version() -> Version {
    ...
    assert_eq!(
        version,
        plist::from_file::<_, Plist>(INFO_PLIST_PATH)
            .expect("failed to read plist for version check")
