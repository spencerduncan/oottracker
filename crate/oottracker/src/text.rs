use {
    ...
    ootr::model::{Dungeon, DungeonReward, DungeonRewardLocation, MainDungeon, Medallion, Stone},
};
...
impl DungeonRewardLocationExt for DungeonRewardLocation {
    fn eat_altar_hint_text(s: &mut &[u8]) -> Option<Self> {
        if eat_any_color_str(s, b"One inside an \x05\x00ancient tree\x05\x0f...")
            || eat_any_color_str(s, b"One in the \x05\x00Deku Tree\x05\x0f...")
        {
            Some(Self::Dungeon(MainDungeon::DekuTree))
        } else if eat_any_color_str(s, b"One within an \x05\x00immense cavern\x05\x0f...")
            || eat_any_color_str(s, b"One in \x05\x00Dodongo's Cavern\x05\x0f...")
        {
            Some(Self::Dungeon(MainDungeon::DodongosCavern))
        } else if eat_any_color_str(s, b"One in the \x05\x00belly of a deity\x05\x0f...")
            || eat_any_color_str(s, b"One in \x05\x00Jabu Jabu's Belly\x05\x0f...")
        {
            Some(Self::Dungeon(MainDungeon::JabuJabu))
        } else if eat_any_color_str(s, b"One in a \x05\x00deep forest\x05\x0f...")
            || eat_any_color_str(s, b"One in the \x05\x00Forest Temple\x05\x0f...")
        {
            Some(Self::Dungeon(MainDungeon::ForestTemple))
        } else if eat_any_color_str(s, b"One on a \x05\x00high mountain\x05\x0f...")
            || eat_any_color_str(s, b"One in the \x05\x00Fire Temple\x05\x0f...")
        {
            Some(Self::Dungeon(MainDungeon::FireTemple))
        } else if eat_any_color_str(s, b"One under a \x05\x00vast lake\x05\x0f...")
            || eat_any_color_str(s, b"One in the \x05\x00Water Temple\x05\x0f...")
        {
            Some(Self::Dungeon(MainDungeon::WaterTemple))
        } else if eat_any_color_str(s, b"One within the \x05\x00house of the dead\x05\x0f...")
            || eat_any_color_str(s, b"One in the \x05\x00Shadow Temple\x05\x0f...")
        {
            Some(Self::Dungeon(MainDungeon::ShadowTemple))
        } else if eat_any_color_str(s, b"One inside a \x05\x00goddess of the sand\x05\x0f...")
            || eat_any_color_str(s, b"One in the \x05\x00Spirit Temple\x05\x0f...")
        {
            Some(Self::Dungeon(MainDungeon::SpiritTemple))
        } else if eat_any_color_str(s, b"One in \x05\x00@'s pocket\x05\x0f...")
            || eat_any_color_str(s, b"One \x05\x00@ already has\x05\x0f...")
        {
            //TODO check for the current player name instead
...
pub(crate) fn read_knowledge(mut text: &[u8]) -> Knowledge {
    ...
    if eat_str(&mut text, b"\x08Princess Ruto got the \x01") {
        if let Some(reward) = DungeonReward::eat_ruto_hint_text(&mut text) {
            knowledge.dungeon_reward_locations.insert(
                reward,
        ...
        &mut text,
        b"\x08\x13\x75You found the \x05\x41Compass\x05\x40\x01for ",
    ) {
