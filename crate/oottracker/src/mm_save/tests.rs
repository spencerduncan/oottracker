...
fn test_from_save_data_parses_sword_shield() {
    ...
    // OoTMM uses u16: sword in low nibble (bits 0-3), shield in next nibble (bits 4-7)
    let equipment: u16 = 0x03 | (0x03 << 4);
    data[SWORD_SHIELD..SWORD_SHIELD + 2].copy_from_slice(&equipment.to_be_bytes());
...
fn test_song_accessors_from_parsed_data() {
    ...
    // Set quest items with Song of Time and Song of Healing (using OoTMM bit layout)
    let quest_items = MmQuestItems::SONG_TIME | MmQuestItems::SONG_HEALING;
    let quest_bits: u32 = quest_items.to_ootmm_bits();
    data[QUEST_ITEMS..QUEST_ITEMS + 4].copy_from_slice(&quest_bits.to_be_bytes());
...
fn test_boss_remains_accessors_from_parsed_data() {
    ...
    // Set quest items with Odolwa and Gyorg remains (using OoTMM bit layout)
    let quest_items = MmQuestItems::REMAINS_ODOLWA | MmQuestItems::REMAINS_GYORG;
    let quest_bits: u32 = quest_items.to_ootmm_bits();
    data[QUEST_ITEMS..QUEST_ITEMS + 4].copy_from_slice(&quest_bits.to_be_bytes());
...
fn test_parse_inventory_empty_from_raw() {
    ...
    for i in 0..24 {
        data[ootmm_offsets::INVENTORY + i] = mm_item_ids::NONE;
    }
...
fn test_parse_all_songs_from_raw() {
    ...
    // Set all song bits using OoTMM bit layout
    let quest_items = MmQuestItems::SONG_AWAKENING
        | MmQuestItems::SONG_GORON
        | MmQuestItems::SONG_ZORA
        | MmQuestItems::SONG_EMPTINESS
        | MmQuestItems::SONG_ORDER
        ...
        | MmQuestItems::SONG_SOARING
        | MmQuestItems::SONG_STORMS
        | MmQuestItems::SONG_LULLABY_INTRO;
    let quest_bits: u32 = quest_items.to_ootmm_bits();
...
fn test_parse_songs_individual_bits() {
    ...
    // Test each song bit individually using OoTMM bit layout
    let songs = [
    ...
    for (song_flag, name) in songs {
        let mut data = vec![0u8; MM_SIZE];
        // Convert to OoTMM bit layout for raw data
        data[QUEST_ITEMS..QUEST_ITEMS + 4]
            .copy_from_slice(&song_flag.to_ootmm_bits().to_be_bytes());
...
fn test_bombers_notebook_from_raw() {
    ...
    let mut data = vec![0u8; MM_SIZE];
    // Use OoTMM bit layout for notebook
    let quest_bits: u32 = MmQuestItems::NOTEBOOK.to_ootmm_bits();
    data[QUEST_ITEMS..QUEST_ITEMS + 4].copy_from_slice(&quest_bits.to_be_bytes());
