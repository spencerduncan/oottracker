...
use {
    ...
    bytes::Bytes,
    derive_more::From,
    futures::{future::FutureExt as _, stream::TryStreamExt as _},
    iced::{
        alignment,
        widget::{
            button::{self, Button},
    ...
    sysinfo::{Pid, ProcessRefreshKind, SystemExt as _},
    tokio::{io, time::sleep},
...
#[cfg(target_arch = "x86")]
const TRACKER_PLATFORM_SUFFIX: &str = "-bizhawk-win32.zip";
#[cfg(target_arch = "x86_64")]
const TRACKER_PLATFORM_SUFFIX: &str = "-bizhawk-win64.zip";

#[cfg(target_arch = "x86_64")]
const BIZHAWK_PLATFORM_SUFFIX: &str = "-win-x64.zip";
...
impl Application for App {
    ...
    fn new(args: Args) -> (Self, Command<Result<Message, Error>>) {
        let cmd = Command::single(Action::Future(
            async move {
    ...
    fn title(&self) -> String {
        format!("updating the OoT auto-tracker…")
    }
    ...
    fn update(&mut self, msg: Result<Message, Error>) -> Command<Result<Message, Error>> {
        match msg {
            ...
            Ok(Message::TrackerResponse(client, response)) => {
                ...
                Command::single(Action::Future(
                    async move {
                        let mut zip_file = StreamReader::new(
                            response
                                .bytes_stream()
                                .map_err(|e| io::Error::new(io::ErrorKind::Other, e)),
                        );
                        let mut zip_file =
                            async_zip::read::stream::ZipFileReader::new(&mut zip_file);
                        let mut required_bizhawk_version = None;
                        while let Some(entry) = zip_file.entry_reader().await? {
                            match entry.entry().name() {
                                "README.txt" => {
                                    let (readme_prefix, _) =
                                        include_str!("../../../assets/bizhawk-readme.txt")
                                            .split_once("{}")
                                            .expect("failed to parse readme template");
                                    required_bizhawk_version = Some(
                                        entry
                                            .read_to_string_crc()
                                            .await?
                                            .strip_prefix(readme_prefix)
                                            .ok_or(Error::ReadmeFormat)?
                                            .split_once(". ")
                                            .ok_or(Error::ReadmeFormat)?
                                            .0
                                            .parse()?,
                                ...
                                "OotAutoTracker.dll" => {
                                    ...
                                    entry
                                        .copy_to_end_crc(
                                            &mut File::create(
                                                external_tools.join("OotAutoTracker.dll"),
                                            )
                                            .await?,
                                            64 * 1024,
                                        )
                                ...
                                "oottracker.dll" => {
                                    ...
                                    entry
                                        .copy_to_end_crc(
                                            &mut File::create(
                                                external_tools.join("oottracker.dll"),
                                            )
                                            .await?,
                                            64 * 1024,
                                        )
                        ...
                        let required_bizhawk_version =
                            required_bizhawk_version.ok_or(Error::MissingReadme)?;
            ...
            Ok(Message::UpdateBizHawk(client, required_version)) => {
                ...
                Command::single(Action::Future(
                    async move {
                        ...
                        let release = Repo::new("TASEmulators", "BizHawk")
                            .release_by_tag(&client, version_str)
                            .await?
                            .ok_or(Error::NoReleases)?;
                        let (asset,) = release
                            .assets
            ...
            Ok(Message::BizHawkResponse(response)) => {
                ...
                Command::single(Action::Future(
                    async move { Ok(Message::BizHawkZip(response.bytes().await?)) }.boxed(),
                ))
            }
            Ok(Message::BizHawkZip(mut response)) => {
                ...
                Command::single(Action::Future(
                    async move {
                        let mut zip_file =
                            async_zip::read::mem::ZipFileReader::new(&mut response).await?;
                        let entries = zip_file
                            .entries()
                            .iter()
                            .enumerate()
                            .map(|(idx, entry)| (idx, entry.dir(), path.join(entry.name())))
                            .collect_vec();
                        for (idx, is_dir, path) in entries {
                            if is_dir {
                                ...
                                zip_file
                                    .entry_reader(idx)
                                    .await?
                                    .copy_to_end_crc(&mut File::create(path).await?, 64 * 1024)
                                    .await?;
                            }
                        }
                        Ok(Message::Launch)
            ...
            Ok(Message::Launch) => {
                ...
                Command::single(Action::Future(
                    async move {
                        let path = unsafe {
                            ...
                            PathBuf::from(if result == 0 {
                                ...
                                if result == 0 {
                                    ...
                                    > u32::try_from(buf.len()).expect("buffer too large")
                                {
                                    panic!("path too long")
                                } else {
                                    OsString::from_wide(
                                        &buf[0..result.try_into().expect("path too long")],
                                    )
                                }
                            } else {
                                drop(path_wide);
                        ...
                        std::process::Command::new(path.join("EmuHawk.exe"))
                            .arg("--open-ext-tool-dll=OotAutoTracker")
                            .current_dir(path)
                            .spawn()?;
                        Ok(Message::Done)
                    }
