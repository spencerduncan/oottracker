use {
    ootr::model::{DungeonReward, DungeonRewardLocation, MainDungeon, Stone},
    oottracker::{
        ui::{CellOverlay, CellRender, CellStyle, ImageInfo, LocationStyle},
        ModelState,
    },
...
impl RestreamState {
            ...
            worlds: worlds
                ...
                .map(|players| {
                    ...
                    (
                        ...
                        players
                            .into_iter()
                            .map(|player| (player.to_owned(), ModelState::default()))
                            .collect(),
                    )
                })
                .collect(),
    ...
    pub(crate) fn runner(&self, runner: &str) -> Option<(&Sender<()>, &Receiver<()>, &ModelState)> {
        self.worlds
            .iter()
            .filter_map(|(tx, rx, players)| {
                players.get(runner).map(move |state| (&*tx, &*rx, state))
            })
            .next()
    }
    ...
    pub(crate) fn runner_mut(
        &mut self,
        runner: &str,
    ) -> Option<(&Sender<()>, &Receiver<()>, &mut ModelState)> {
        self.worlds
            .iter_mut()
            .filter_map(|(tx, rx, players)| {
                players
                    .get_mut(runner)
                    .map(move |state| (&*tx, &*rx, state))
            })
            .next()
    }
}

pub(crate) fn render_double_cell(
    runner1: &ModelState,
    runner2: &ModelState,
    ...
    let img_filename = match reward {
        DungeonReward::Medallion(med) => {
            Cow::Owned(format!("{}_medallion", med.element().to_ascii_lowercase()))
        }
    ...
    let style = match (
        runner1.ram.save.quest_items.has(reward),
        runner2.ram.save.quest_items.has(reward),
    ) {
    ...
    let location = (runner1.knowledge.clone() & runner2.knowledge.clone())
        .map(|knowledge| knowledge.dungeon_reward_locations.get(&reward).copied())
        .unwrap_or_default(); //TODO display contradiction errors differently?
